...
from tensorflow.keras.layers import LSTM, Bidirectional, Dense, Flatten, Conv1D, BatchNormalization, LeakyReLU, ELU, ReLU
...
#initializer = tf.keras.initializers.Orthogonal(gain=0.5, seed=None)
...
def Generator(input_dim, output_dim, feature_size) -> tf.keras.models.Model:
    ...
    #model.add(Bidirectional(LSTM(units=256, return_sequences=True), input_shape=(input_dim, feature_size)))
...
def Discriminator() -> tf.keras.models.Model:
    ...
    model.add(Conv1D(16, input_shape=(37, 1), kernel_size=3, strides=2, activation=LeakyReLU(alpha=0.01)))
    model.add(Conv1D(64, kernel_size=3, strides=2,  activation=LeakyReLU(alpha=0.01)))
    #model.add(BatchNormalization())
    model.add(Conv1D(128, kernel_size=3, strides=2,  activation=LeakyReLU(alpha=0.01)))
    #model.add(BatchNormalization())
    model.add(Flatten())
...
class GAN():
    ...
    def gradient_penalty(self, batch_size, real_output, fake_output):
        ...
        alpha = tf.random.normal([batch_size, 37, 1], 0.0, 1.0)
