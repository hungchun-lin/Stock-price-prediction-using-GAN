...
if __name__ == '__main__':
    ...
    generator = make_generator_model(X_train.shape[1], X_train.shape[2])
    discriminator = make_discriminator_model()
    gan = GAN(generator, discriminator)
    Predicted_price, Real_price = gan.train(X_train, y_train, epoch)
...
# Calculate RMSE
predicted_mean = predict_result['predicted_mean']
real_mean = real_price['real_mean']
For_MSE = pd.concat([predicted_mean, real_mean], axis = 1)
RMSE = np.sqrt(mean_squared_error(predicted_mean, real_mean))
print('-- RMSE -- ', RMSE)
